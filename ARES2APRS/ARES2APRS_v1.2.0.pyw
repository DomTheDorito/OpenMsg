...
class APRSFIUploader:
    ...
    alt = {'emergency': '!', 'crash_site': "'", 'snow': '*',
        ...
        'dust': 'b', 'civ': 'c', 'funnel': 'f', 'gale': 'g',
        'tornado': 't', 'flood': 'w', 'skywarn': 'y'}
    
    def __init__(self, root):
        self.root = root
        self.root.title("ARES2APRS v1.2 by N1OF/AD8AK")
        ...
        self.listbox.grid(row=6, column=0, padx=10, sticky="w")
        ...
        self.image_preview_label.grid(row=6, column=1, padx=10, pady=10)
    ...
    def on_selection_change(self,event):
        ...
        if selected_index:
            ...
            # Load image preview
            icon_path = os.path.join("symbols", f"{self.selected_key}.png")  # Icon folder must be in same directory!
            if os.path.exists(icon_path):
                image = Image.open(icon_path)
                #image = image.resize((50, 50), Image.ANTIALIAS)  # Resize image
                photo = ImageTk.PhotoImage(image)
                self.image_preview_label.configure(image=photo)
                self.image_preview_label.image = photo  # Keep reference to avoid garbage collection
            else:
                self.image_preview_label.configure(image=None)  # Clear image preview if icon not found
